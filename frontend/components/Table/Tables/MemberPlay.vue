    ...
    <a-row>
      ...
      <a-row class="p-2" :span="24">
        ...
        <a-col class="p-1" :span="2">
          <label><br></label>
          <a-button class="sky" type="primary" @click="search"><SearchOutlined /> ค้นหา</a-button>
        </a-col>
      </a-row>
    </a-row>
        <a-row class="p-2 memberplay-100">
...
<script lang="ts" setup>
    ...
    import { getReportGameByIdService } from '~/services/reportGame';
...
const getReportGame = async() => {
  loading.value = true;
  const data = await getReportGameByIdService(formData,props.id);
