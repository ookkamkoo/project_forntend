...
import { getMiniGameReportByIdServices } from '~/services/reportGame';
...
const handleDateSelectChange = () => {
  ...
  getReportSummaryByIdServices();
};

const getReportSummaryByIdServices = async() => {
  loading.value = true;
  const data = await getMiniGameReportByIdServices(formData,props.id);
